...
Requires the GEMINI_API_KEY environment variable to be set.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("Processing audio files in: %s\n", audioDir)
		fmt.Printf("Using model: %s\n", modelName)
		processAudioDirectory(apiKey, audioDir, modelName, forceOverwrite)
	},
}
...
func init() {
	...
	// Define flags
	sttCmd.Flags().StringVarP(&audioDir, "dir", "", "", "Directory containing audio files (required)")
	sttCmd.Flags().BoolVarP(&forceOverwrite, "force", "", false, "Overwrite existing .txt transcript files")
	sttCmd.Flags().StringVarP(&modelName, "model", "", "gemini-2.5-flash", "The model to use for transcription")

	// Mark 'dir' as a required flag
	if err := sttCmd.MarkFlagRequired("dir"); err != nil {
		log.Fatalf("Failed to mark 'dir' flag as required: %v", err)
	}
}

// --- Core Logic ---

func processAudioDirectory(apiKey, dir, model string, force bool) {
	...
	for _, file := range files {
		...
		audioFilePath := filepath.Join(dir, fileName)
		...
		// 2. Call Gemini API
		transcript, err := getTranscript(httpClient, apiKey, model, audioData, mimeType)
		if err != nil {
...
func getTranscript(client *http.Client, apiKey, modelName string, audioData []byte, mimeType string) (string, error) {
	...
	// 3. Build the URL
	url := fmt.Sprintf("%s%s:generateContent?key=%s", apiURLBase, modelName, apiKey)
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if err != nil {
			return "", fmt.Errorf("failed to create HTTP request: %v", err) // Non-retryable
		}
		...
		case http.StatusOK:
			...
			resp.Body.Close()
			if err != nil {
				return "", fmt.Errorf("failed to read successful API response body: %v", err)
			}
	...
	return "", fmt.Errorf("all %d retry attempts failed. Last error: %v", maxRetries+1, lastErr)
}
